...
parser.add_argument("--steps", type=int, default=80, help="diffusion steps")
parser.add_argument(
    "--noise_schedule",
    type=str,
    default="linear-var",
...
if n_hop == 2:
    sec_hop = torch.load(
        os.path.join(args.data_path, args.dataset, "two_hop_rates_items.pt")
    )
    multi_hop = sec_hop
elif n_hop == 3:
    multi_hop = torch.load(
        os.path.join(args.data_path, args.dataset, "two_hop_rates_items.pt")
    )

train_data, test_y_data, n_user, n_item = data_utils.data_load(train_path, test_path)
...
train_loader_sec_hop = DataLoader(
    multi_hop,
    batch_size=args.batch_size,
    pin_memory=True,
    shuffle=False,
...
def evaluate(data_loader, data_loader_sec_hop, data_te, mask_his, topN):
    ...
    with torch.no_grad():
        for (batch_idx, batch), (batch_idx_2, batch_2) in zip(
            enumerate(data_loader), enumerate(data_loader_sec_hop)
        ):
            his_data = mask_his[
                e_idxlist[
            ...
            prediction = diffusion.p_sample(
                model, batch, batch_2, args.sampling_steps, args.sampling_noise
            )
...
if __name__ == "__main__":
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (batch_idx, batch), (batch_idx_2, batch_2) in zip(
            enumerate(train_loader), enumerate(train_loader_sec_hop)
        ):
            batch = batch.to(device)
            batch_2 = batch_2.to(device)
            batch_count += 1
