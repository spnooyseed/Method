...
class CAM_AE_multihops(nn.Module):
    ...
    def __init__(
        ...
        self.forward_layers = nn.ModuleList(
            [nn.Linear(d_model, d_model) for i in range(num_layers)]
        )

        self.dim_inters = 512  # The hidden dimension, correcting to k in the paper
        self.first_hop_embedding = nn.Linear(
            1, d_model
        )  # Expend dimension, correcting to d in the paper
    ...
    def forward(self, x, x_sec_hop, timesteps):
        ...
        x = self.encoder(x)
        h_sec_hop = self.encoder(
            x_sec_hop[:, 0 : self.in_dims]
        )  # self.encoder(x_sec_hop[:, 0:6969]) #x_sec_hop[:, 0:2810]
        h_third_hop = self.encoder(
            x_sec_hop[:, self.in_dims :]
        )  # self.encoder(x_sec_hop[:, 6969:])
