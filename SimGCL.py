...
class Coach:
    ...
    def run(self):
        self.prepareModel()
        log("SimGCL Model Prepared")
        if args.load_model:
            self.loadModel()
            stloc = len(self.metrics["TrainLoss"]) * args.tstEpoch - (args.tstEpoch - 1)
        else:
            stloc = 0
            log("SimGCL Model Initialized")
        curTime = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        fileName = f"SimGCL_{args.data}-{curTime}_pid_{os.getppid()}"
    ...
    def trainEpoch(self, ep):
        ...
        for i, tem in enumerate(trnLoader):
            ...
            usrEmbeds_1, itmEmbeds_1 = self.model(self.handler.torchBiAdj, True)
            usrEmbeds_2, itmEmbeds_2 = self.model(self.handler.torchBiAdj, True)
            cl_loss = (
                InfoNce(usrEmbeds_1[ancs_unique], usrEmbeds_2[ancs_unique], 0.1)
                + InfoNce(itmEmbeds_1[pos_unique], itmEmbeds_2[pos_unique], 0.1)
            ) * 0.1
            loss = BPRloss + regLoss + cl_loss
            epLoss += loss.item()
