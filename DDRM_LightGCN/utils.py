...
class BPRLoss:
    ...
    def drop_rate_schedule(self, iteration):
        drop_rate = np.linspace(
            0,
            self.config["drop_rate"] ** self.config["exponent"],
            self.config["num_gradual"],
        )
    ...
    def call_bpr(self, users, pos, neg, iter):
        loss, reg_loss, re_loss, pos_score = self.model.bpr_loss(
            users,
            pos,
            neg,
            self.user_reverse_model,
            self.item_reverse_model,
...
def getFileName():
    ...
    return (
        os.path.join(world.FILE_PATH, file),
        os.path.join(world.FILE_PATH, user_model),
        os.path.join(world.FILE_PATH, item_model),
    )
