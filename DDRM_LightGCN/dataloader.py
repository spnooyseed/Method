...
class DiffData(BasicDataset):
    ...
    def __init__(self, config=world.config, path=None):
        ...
        with open(train_file, "rb") as fs:
            load_tmp = (pickle.load(fs) != 0).astype(np.float32)
        if type(load_tmp) is not sp.coo_matrix:
            load_tmp = sp.coo_matrix(load_tmp)
        self.train_list = np.column_stack((load_tmp.row, load_tmp.col))
        self.n_user, self.m_item = load_tmp.shape
        with open(test_file, "rb") as fs:
            load_tmp = (pickle.load(fs) != 0).astype(np.float32)
        if type(load_tmp) is not sp.coo_matrix:
            load_tmp = sp.coo_matrix(load_tmp)
        self.test_list = np.column_stack((load_tmp.row, load_tmp.col))
    ...
    def get_pair_bpr(self):
        """
        适配“每个 epoch 调用”：每次调用重新生成样本，确保列表长度 == self.traindataSize
        解决 IndexError: list index out of range
        """
        # 每次调用先清空之前的样本列表（避免多个 epoch 样本累积）
        self.user = []
        self.posItem = []
        self.negItem = []

        # 循环生成样本，直到列表长度达到训练集交互总数（self.traindataSize）
        while len(self.user) < self.traindataSize:
            # 1. 随机选择一个用户（范围：0 ~ self.n_user - 1）
            user = np.random.randint(0, self.n_user)
            # 2. 获取该用户的所有正样本（从预计算的 self._allPos 中取）
