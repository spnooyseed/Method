...
# define dataset
args = parse_args()
dataset = dataloader.DiffData(path=join(args.data_path, args.dataset))
...
user_reverse_model = register.DIFF_MODELS["dnn"](
    in_dims, out_dims, args.emb_size, time_type="cat", norm=args.norm
)
user_reverse_model = user_reverse_model.to(world.device)
...
# define bpr
bpr = utils.BPRLoss(
    Recmodel, user_reverse_model, item_reverse_model, diffusion, world.config
)

weight_file, user_weight_file, item_weight_file = utils.getFileName()
print(f"load and save to {weight_file}")

path = "./pretrain_checkpoint/LightGCN_" + args.dataset + ".ckpt"
Light_gcn = torch.load(path, map_location=torch.device("cuda"))
Recmodel.embedding_user.weight = torch.nn.Parameter(
    Light_gcn["uEmbeds"].to(Recmodel.embedding_user.weight.device)
)
Recmodel.embedding_item.weight = torch.nn.Parameter(
    Light_gcn["iEmbeds"].to(Recmodel.embedding_user.weight.device)
)
...
try:
    ...
    for epoch in range(world.TRAIN_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            if results[5][0] > best_recall:
                best_epoch = epoch
                best_recall = results[5][0]
                best_v = results
            ...
            if epoch == 30:
                recall_list.append((epoch, results[5][0]))
    ...
    user_reverse_model.load_state_dict(
        torch.load(user_weight_file, map_location=torch.device("cpu"))
    )
    item_reverse_model.load_state_dict(
        torch.load(item_weight_file, map_location=torch.device("cpu"))
    )
    ...
    best_results_test = Procedure.Test_all(
        dataset,
        Recmodel,
        user_reverse_model,
        item_reverse_model,
