...
class LightGCN(BasicModel):
    ...
    def computer(self, diff_model, user_reverse_model, item_reverse_model, user, pos):
        ...
        noise_user_emb, noise_item_emb, ts, pt = self.apply_noise(
            ori_user_emb, ori_item_emb, diff_model
        )
        # reverse
        ...
        # get recons loss
        user_recons = diff_model.get_reconstruct_loss(
            ori_user_emb, user_model_output, pt
        )
        item_recons = diff_model.get_reconstruct_loss(
            ori_item_emb, item_model_output, pt
        )
    ...
    def computer_infer(
        self, user, allPos, diff_model, user_reverse_model, item_reverse_model
    ):
        """
        propagate methods for lightGCN
        """
        ...
        for i in indices:
            ...
            out = diff_model.p_mean_variance(
    ...
    def apply_noise_sample(self, cat_emb, diff_model):
        t = torch.tensor([self.config["sampling_steps"] - 1] * cat_emb.shape[0]).to(
            cat_emb.device
        )
    ...
    def apply_T_noise(self, cat_emb, diff_model):
        t = torch.tensor([self.config["steps"] - 1] * cat_emb.shape[0]).to(
            cat_emb.device
        )
        noise = torch.randn_like(cat_emb)
        noise_emb = diff_model.q_sample(cat_emb, t, noise)
    ...
    def getUsersRating(
        self, users, train_dict, user_reverse_model, item_reverse_model, diff_model
    ):
        item_emb, all_items = self.computer_infer(
            users, train_dict, diff_model, user_reverse_model, item_reverse_model
        )
    ...
    def rounding_cos(self, item_emb, all_items):
        ...
        all_items_normalized = F.normalize(
            all_items, p=2, dim=1
        )  # Shape: [item_num, emb]
    ...
    def getEmbedding(
        ...
        users_emb, pos_emb, recons_loss, all_items = self.computer(
            diff_model, user_reverse_model, item_reverse_model, users, pos_items
        )
    ...
    def bpr_loss(
        self, users, pos, neg, user_reverse_model, item_reverse_model, diff_model
    ):
        (users_emb, pos_emb, neg_emb, userEmb0, posEmb0, negEmb0, reconstruct_loss) = (
            self.getEmbedding(
                users.long(),
                pos.long(),
                neg.long(),
...
class Diff_Transformer(nn.Module):
    def __init__(
        self, in_dims, out_dims, w_in_dims, w_out_dims, norm=False, dropout=0.5
    ):
        ...
        self.wk_in_layers = nn.ModuleList(
            [
                nn.Linear(d_in, d_out)
                for d_in, d_out in zip(w_in_dims_temp[:-1], w_in_dims_temp[1:])
            ]
        )
        self.wk_out_layers = nn.ModuleList(
            [
                nn.Linear(d_in, d_out)
        ...
        self.wv_in_layers = nn.ModuleList(
            [
                nn.Linear(d_in, d_out)
                for d_in, d_out in zip(w_in_dims_temp[:-1], w_in_dims_temp[1:])
            ]
        )
        self.wv_out_layers = nn.ModuleList(
            [
                nn.Linear(d_in, d_out)
                for d_in, d_out in zip(w_out_dims_temp[:-1], w_out_dims_temp[1:])
            ]
        ...
        self.wq_in_layers = nn.ModuleList(
            [
                nn.Linear(d_in, d_out)
                for d_in, d_out in zip(w_in_dims_temp[:-1], w_in_dims_temp[1:])
            ]
        )
        self.wq_out_layers = nn.ModuleList(
            [
                nn.Linear(d_in, d_out)
                for d_in, d_out in zip(w_out_dims_temp[:-1], w_out_dims_temp[1:])
            ]
    ...
    def forward(self, noise_emb, con_emb, timesteps):
        # noise emb: [bs, emb]
        time_emb = timestep_embedding(timesteps, world.config["latent_dim_rec"]).to(
            noise_emb.device
        )
